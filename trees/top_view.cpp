...
void print_top_view(Node *temp, int level, int hd, map<int, pair<int, int>> &m)
{
    if (temp == NULL)
        return;
    ...
    print_top_view(temp->left, level + 1, hd - 1, m);
    print_top_view(temp->right, level + 1, hd + 1, m);
}

void BST::top_view(Node *root)
{
    map<int, pair<int, int>> mp;

    print_top_view(root, 0, 0, mp);
